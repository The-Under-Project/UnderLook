...
namespace Health
{
    public class AIstatsManager : Photon.PunBehaviour, IPunObservable
    {
        [Tooltip("The current Health of our IA")]
        ...
        public void Update()
        {
            ...
            if (photonView.isMine)
            {
                if (this.GetComponent<Player.AI>().hp <= 0f)
                {
                    Debug.Log("Health negative");
                    //Destroy(gameObject);
                }
            }
            Health = GetComponent<Player.AI>().hp;
        }
        public void Damage()
        {
            GetComponent<Player.AI>().hp -= 50;
