...
namespace Health
{
    public class PlayerManagerRasla: Photon.PunBehaviour, IPunObservable
    {
        ...
        public void Update()
        {
            ...
            {
                GetComponent<Player.Rasla>().hp -= 50;
            }
            if (photonView.isMine)
            {
                if (this.GetComponent<Player.Rasla>().hp <= 0f)
                {
                    Debug.Log("Death");
                    GetComponent<Player.Rasla>().hp = GetComponent<Player.Rasla>().hpmax;
                    GetComponent<Moving>().TP(GetComponent<TeamColor>().isBlue);
                }
            }
            Health = GetComponent<Player.Rasla>().hp;
        }
        public void Damage()
        {
            GetComponent<Player.Rasla>().hp -= 50;
